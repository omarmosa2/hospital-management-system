...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-slate-100">{{ __('doctors') }}</h1>
            <p class="text-gray-600 dark:text-slate-400 mt-2">{{ __('manage_doctors_info') }}</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg dark:shadow-slate-900">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            <div class="flex-1 md:mr-6">
                <div class="relative">
                    <input type="text" placeholder="{{ __('search_for_doctor') }}" class="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100 placeholder-gray-400 dark:placeholder-slate-400">
            ...
            <div class="flex space-x-4">
                <select class="px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100">
                    <option>{{ __('all_doctors') }}</option>
                    <option>{{ __('available') }}</option>
                    <option>{{ __('not_available') }}</option>
                </select>
                <select class="px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg dark:shadow-slate-900 overflow-hidden">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors duration-200">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <div class="mr-4">
                                            ...
                                            <div class="text-xs text-gray-500 dark:text-slate-400">{{ __('doctor') }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900">
                                        ...
                                        {{ $doctor->clinic->name ?? __('not_specified') }}
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <form action="{{ route('doctors.destroy', $doctor) }}" method="POST" class="inline-block" onsubmit="return confirm('{{ __('confirm_delete_doctor') }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
        ...
        @else
            <div class="p-12 text-center">
                ...
                <h3 class="text-xl font-semibold text-gray-900 mb-2">{{ __('no_doctors') }}</h3>
                <p class="text-gray-600 mb-6">{{ __('no_doctors_yet') }}</p>
