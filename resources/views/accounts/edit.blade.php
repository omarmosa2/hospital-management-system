...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-slate-100">{{ __('edit_account') }}</h1>
            <p class="text-gray-600 dark:text-slate-400 mt-2">{{ __('edit_info') }}: {{ $user->name }}</p>
        </div>
            ...
            <div class="border-b border-gray-200 dark:border-slate-700 pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="password" 
                               ...
                               placeholder="{{ __('leave_empty_keep_current') }}">
                        @error('password')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mt-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                    <div class="flex">
                        ...
                        <div class="mr-3">
                            <h3 class="text-sm font-medium text-yellow-800 dark:text-yellow-300">{{ __('important_note') }}</h3>
                            <div class="mt-2 text-sm text-yellow-700 dark:text-yellow-300">
                                <p>{{ __('if_password_empty_keep_current') }}</p>
                                <p>{{ __('current_password') }}: <span class="font-mono bg-gray-100 dark:bg-slate-700 px-2 py-1 rounded">•••••••• ({{ __('encrypted') }})</span></p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach($roles as $role)
                            <div class="flex items-center">
                                ...
                                <label for="role_{{ $role->id }}" class="mr-3 text-sm font-medium text-gray-700 dark:text-slate-300">
