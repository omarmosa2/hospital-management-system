...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-white rounded-2xl p-6 shadow-lg card-hover border-l-4 border-blue-500">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-gray-600 text-sm font-medium">إجمالي المرضى</p>
                <p class="text-3xl font-bold text-gray-900 mt-2">{{ $stats['total_patients'] ?? 0 }}</p>
    ...
    <div class="bg-white rounded-2xl p-6 shadow-lg card-hover border-l-4 border-green-500">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-gray-600 text-sm font-medium">إجمالي الأطباء</p>
                <p class="text-3xl font-bold text-gray-900 mt-2">{{ $stats['total_doctors'] ?? 0 }}</p>
                <p class="text-green-600 text-sm mt-1">
    ...
    <div class="bg-white rounded-2xl p-6 shadow-lg card-hover border-l-4 border-purple-500">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-900 mt-2">${{ number_format($stats['total_revenue'] ?? 0, 2) }}</p>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
    ...
    <div class="bg-white rounded-2xl p-6 shadow-lg">
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-xl font-bold text-gray-900">نظرة عامة على المواعيد</h3>
            <div class="flex space-x-2">
                <button class="px-3 py-1 bg-blue-100 text-blue-600 rounded-full text-sm font-medium">هذا الشهر</button>
                <button class="px-3 py-1 text-gray-500 rounded-full text-sm font-medium">هذا الأسبوع</button>
            </div>
        </div>
        <div class="h-64">
    ...
    <div class="bg-white rounded-2xl p-6 shadow-lg">
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-xl font-bold text-gray-900">الإيرادات الشهرية</h3>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    @foreach($stats['recent_appointments'] as $appointment)
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors duration-200">
                            ...
                            <div>
                                    ...
                                    <p class="text-sm text-gray-600">{{ $appointment->doctor->user->name ?? 'غير محدد' }}</p>
    ...
    <div class="bg-white rounded-2xl shadow-lg">
        <div class="p-6 border-b border-gray-100">
            <div class="flex items-center justify-between">
                <h3 class="text-xl font-bold text-gray-900">الفواتير الأخيرة</h3>
                ...
                <div class="space-y-4">
                    @foreach($stats['recent_bills'] as $bill)
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors duration-200">
                            ...
                            <div>
                                    <p class="font-semibold text-gray-900">{{ $bill->patient->user->name ?? 'غير محدد' }}</p>
                                    <p class="text-sm text-gray-600">فاتورة #{{ $bill->bill_number }}</p>
                                    <p class="text-xs text-gray-500">{{ $bill->created_at->format('d M Y') }}</p>
                                </div>
                            </div>
                            <div class="text-left">
                                <p class="font-bold text-gray-900">${{ number_format($bill->total_amount, 2) }}</p>
                                <span class="px-3 py-1 text-xs rounded-full font-medium
