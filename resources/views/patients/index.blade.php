...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-slate-100">{{ __('patients') }}</h1>
            <p class="text-gray-600 dark:text-slate-400 mt-2">{{ __('manage_patients_info') }}</p>
        </div>
        @can('create patients')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-gradient-to-r from-blue-500 to-blue-600 dark:from-blue-900 dark:to-blue-800 rounded-2xl p-6 text-white shadow-lg dark:shadow-slate-900">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-blue-100 dark:text-blue-300 text-sm font-medium">{{ __('total_patients') }}</p>
        ...
        <div class="bg-gradient-to-r from-green-500 to-green-600 dark:from-green-900 dark:to-green-800 rounded-2xl p-6 text-white shadow-lg dark:shadow-slate-900">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-green-100 dark:text-green-300 text-sm font-medium">{{ __('males') }}</p>
        ...
        <div class="bg-gradient-to-r from-pink-500 to-pink-600 dark:from-pink-900 dark:to-pink-800 rounded-2xl p-6 text-white shadow-lg dark:shadow-slate-900">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-pink-100 dark:text-pink-300 text-sm font-medium">{{ __('females') }}</p>
        ...
        <div class="bg-gradient-to-r from-purple-500 to-purple-600 dark:from-purple-900 dark:to-purple-800 rounded-2xl p-6 text-white shadow-lg dark:shadow-slate-900">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-purple-100 dark:text-purple-300 text-sm font-medium">{{ __('total_appointments') }}</p>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg dark:shadow-slate-900 overflow-hidden">
        <div class="p-6 border-b border-gray-200 dark:border-slate-700">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-slate-100">{{ __('patients_list') }}</h3>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                <thead class="bg-gray-50 dark:bg-slate-700">
                    <tr>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">{{ __('patient') }}</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">{{ __('medical_record_number') }}</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">{{ __('age') }}</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">{{ __('gender') }}</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">{{ __('phone') }}</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">{{ __('appointments') }}</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">{{ __('actions') }}</th>
                    </tr>
                </thead>
                <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                    @forelse($patients as $patient)
                        <tr class="hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 dark:text-slate-100">{{ \Carbon\Carbon::parse($patient->date_of_birth)->age }} {{ __('years') }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200">
                                    {{ $patient->appointments_count }} {{ __('appointment') }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    <a href="{{ route('patients.show', $patient) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300" title="{{ __('view') }}">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    @can('edit patients')
                                    <a href="{{ route('patients.edit', $patient) }}" class="text-green-600 dark:text-green-400 hover:text-green-900 dark:hover:text-green-300" title="{{ __('edit') }}">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    @endcan
                                    @can('delete patients')
                                    <form action="{{ route('patients.destroy', $patient) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('confirm_delete_patient') }}')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300" title="{{ __('delete') }}">
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900 dark:text-slate-100 mb-2">{{ __('no_patients') }}</h3>
                                    <p class="text-gray-500 dark:text-slate-400 mb-6">{{ __('no_patients_registered_yet') }}</p>
                                    @can('create patients')
                                    <a href="{{ route('patients.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 dark:bg-blue-700 text-white rounded-lg hover:bg-blue-700 dark:hover:bg-blue-600 transition-colors duration-200">
                                        <i class="fas fa-plus mr-2"></i>
                                        {{ __('add_first_patient') }}
                                    </a>
                                    @endcan
                                </div>
                            </td>
