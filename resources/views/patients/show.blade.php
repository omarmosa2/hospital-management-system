...
@section('title', __('patient_details'))
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">{{ __('patient_details') }}</h1>
            <p class="text-gray-600 mt-2">{{ __('detailed_patient_information') }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="text-center mb-6">
                    ...
                    <p class="text-sm text-gray-500">{{ \Carbon\Carbon::parse($patient->date_of_birth)->age }} {{ __('years') }}</p>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center text-gray-600">
                        ...
                        <span>{{ $patient->gender == 'male' ? __('male') : __('female') }}</span>
                    </div>
                    <div class="flex items-center text-gray-600">
                ...
                @if($patient->emergency_contact_name)
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        <h3 class="text-lg font-semibold text-gray-900 mb-3">{{ __('emergency_contact') }}</h3>
                        <div class="space-y-2">
                            <div class="flex items-center text-gray-600">
        ...
        <div class="lg:col-span-2 space-y-6">
            <!-- Medical Information -->
            <div class="bg-white rounded-2xl shadow-lg p-6">
                <h3 class="text-xl font-bold text-gray-900 mb-4">{{ __('medical_information') }}</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <h4 class="font-semibold text-gray-900 mb-2">{{ __('medical_history') }}</h4>
                        <p class="text-gray-600 text-sm">
                            {{ $patient->medical_history ?: __('no_medical_history_recorded') }}
                        </p>
                    </div>
                    <div>
                        <h4 class="font-semibold text-gray-900 mb-2">{{ __('allergies') }}</h4>
                        <p class="text-gray-600 text-sm">
                            {{ $patient->allergies ?: __('no_allergies_recorded') }}
                        </p>
                    </div>
                </div>
                ...
                @if($patient->address)
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        <h4 class="font-semibold text-gray-900 mb-2">{{ __('address') }}</h4>
                ...
                <div class="bg-white rounded-2xl shadow-lg p-6">
                    <div class="flex items-center">
                        ...
                        <div class="mr-4">
                            ...
                            <p class="text-gray-600 text-sm">{{ __('prescriptions') }}</p>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    @forelse($patient->appointments->take(5) as $appointment)
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                            <div class="flex items-center space-x-4">
                                ...
                                <div>
                                    <p class="font-semibold text-gray-900">{{ $appointment->doctor->user->name ?? __('not_specified') }}</p>
                ...
                <div class="space-y-4">
                    @forelse($patient->medicalRecords->take(3) as $record)
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                            ...
                            <div class="text-left">
                                <p class="text-sm text-gray-500">{{ $record->doctor->user->name ?? __('not_specified') }}</p>
                            </div>
                        </div>
                    @empty
                        <div class="text-center py-8">
                            <i class="fas fa-file-medical text-gray-300 text-4xl mb-4"></i>
                            <p class="text-gray-500">{{ __('no_medical_records') }}</p>
                        </div>
                    @endforelse
                </div>
            </div>
